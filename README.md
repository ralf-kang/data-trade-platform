...
- **결측치 · 이상치 조회 및 수정 요청 (`/admin/data/[formId]/quality`)**
...
- **데이터 구조 관계도 (`/super-admin/data-map`)**
  - 이 시스템 자체의 PostgreSQL 스키마(User/FormRegistry/Campaign 등 실제 FK)를 마인드맵으로 시각화하는 내부 유틸리티
...
- **외부 응답 페이지 (`/q/[formId]`)**
  ...
  - 응답자 신원 요구 수준(`identityMode`): 익명 / 식별(개인화 링크) / 인증(로그인) / 혼합
...
| **마스킹 계층** (핵심 방어선) | `FormRegistry.authorHadPrivacyAuth`는 **제작 시점에 고정**되고 사후 자격 취득으로 소급 해제되지 않음. 무자격 상태로 만든 양식은 자유서술형 문항 + k=1 레코드가 조회 화면·CSV 추출·외부 v1 API **세 경로 모두**에서 서비스 계층에 마스킹됨(화면만 가리면 API로 우회되므로). 예외는 슈퍼관리자가 사유를 남기고 명시적으로 해제해야만 적용 |
...
- **시각화**: `@isoterik/react-word-cloud`(워드클라우드 렌더링) — 그 외 관계도·캔버스는 자체 SVG로 구현
...
- **데이터 품질 5단계 잔여 항목** (`docs/데이터품질-검증구간-설계.md` §5): LDAP 자동 채움(읽기 전용 필드), 응답 화면 실시간 형식 검증 확장(의미 타입), 값 사전 기반 유사값 제안, 데이터 추출 시 대표성(응답률·편중) 경고.
