...
- **의미 타입 기반 서버 검증 확장** (`docs/데이터품질-검증구간-설계.md` §5 순위2, 유일한 잔여 항목): 기존 `validateRow`(필수·정규식·선택지·숫자)에 문항의 "의미"(이메일/전화번호/사업자번호 등) 기반 검증을 얹는 것 — 문서가 "이미 있는 것에 얹는 것이라 저비용"이라 명시한 지점.
